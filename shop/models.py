...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    is_staff = models.BooleanField(
        verbose_name="Xodimlik statusi",
        default=False
    )

    is_superuser = models.BooleanField(
        verbose_name="Superadminlik statusi",
        default=False
    )

    is_active = models.BooleanField(
        verbose_name="Profil aktivligi holati",
        default=True
    )
...
class Cart(models.Model):
    ...
    def __str__(self):
        return (
            ...
            f"{self.product.name} - "
            f"{self.count} ta - "
            f"{self.created.strftime('%d/%m/%Y, %H:%M:%S')}"
        )
